//Muestra las lineas de autobuses en la capa.
function initMenu() {
  //HTML para mostrar las lineas
  var html = '<div>';
  for(var i = 0; i < lineNumbers.length; i++) {
    ...
    html += '<label for="lines_checkbox_' + lineNumbers[i] + '">Linea ' + lineNumbers[i] + 'nombrelinea</label>';
    html += '<br />';
  }
  html += '</div>';
...
function onLinesCheckboxClick(checkbox) {
  ...
  map.getLayersByName("Linea " + number)[0].setVisibility(checkbox.checked);
