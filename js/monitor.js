  ...
  var clickControl = new OpenLayers.Control.SelectFeature([stopsLayer, positionsLayer], {
    clickFeature: function(feature) { // onSelect kann nicht verwendet werden (Features werden bereits bei "hover" selektiert)
      ...
      if(feature.layer == stopsLayer) { // es wurde eine Haltestelle ausgewählt
        title = "Parada " + feature.attributes.name;
        $('#monitor_dialog').html("<strong>proximas salidas:</strong><br />");
        OpenLayers.Request.GET({
          url: "proximas_salidas.php?id=" + feature.attributes.id,
        ...
        title = "Linea " + getLineNumberForLfid(feature.attributes.lfid); // + " / Kurs " + feature.attributes.course;
        $('#monitor_dialog').html("<strong>proximas paradas:</strong><br />");
        OpenLayers.Request.GET({
      ...
      $('#monitor_dialog').append('<img id="loading_img" src="imagenes/cargando.gif" width="100" height="100" border="0" alt="Cargando ..."/>');
